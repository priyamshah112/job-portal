...
$(function () {
    ...
    var dtCategoryTableInit = dtCategoryTable.DataTable({
      ...
      fnDrawCallback: function() {        
        ...
        $('.formEdit').on('click', function(){ 
          editForm = true  
          if(editForm){            
            $('#exampleModalLabel').text('Edit Permission');  
          }        
          let id = $(this).attr('category_id');
          selectedRowDataId = id;
          $('#modals-slide-in').modal('show');
          $.ajax({
            url: `${assetPath}api/v1/permissions/${id}`,
            type: 'GET',
            data: {
              _token: token
            },
            ...
            if (result.value) {
              $.ajax({
                ...
                success: function (res) {
                  ...
                  toastr['warning']('👋 Permission Deleted Successfully', 'Deleted!', {
                    closeButton: true,
                    tapToDismiss: false,
                    rtl: isRtl
                  });
  ...
  if (newCategoryForm.length) {
    ...
    newCategoryForm.on('submit', function (e) {
      ...
      if (isValid) {
        ...
        if(editForm){
          $.ajax({
            url: `${assetPath}api/v1/permissions/${selectedRowDataId}?_method=PUT`,
            type: 'POST',
            ...
            success: function (res) {
              ...
              dtCategoryTableInit.ajax.reload();
              clearForm()
              toastr['info']('👋 Permission Updated Successfully.', 'Updated!', {
                closeButton: true,
                tapToDismiss: false,
                rtl: isRtl
              });
  ...
  $('[data-dismiss="modal"]').on('click', function(){      
    if(editForm){            
      $('#exampleModalLabel').text('New Permission');  
      editForm = false;      
