...
$(function () {
    ...
    var dtCategoryTableInit = dtCategoryTable.DataTable({
      ...
      columnDefs: [
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            return (
              '<img class="img-fluid img-rounded" src='+ full['img_path'] +' />'
            );
          }
        },
        {
      ...
      fnDrawCallback: function() {
        ...
        $('.formEdit').on('click', function(){ 
          ...
          editForm = true  
          if(editForm){            
            $('#exampleModalLabel').text('Edit Advert');  
          }        
          let id = $(this).attr('category_id');
          selectedRowDataId = id;
          $('#modals-slide-in').modal('show');
          $.ajax({
            url: `${assetPath}api/v1/vert/${id}`,
            type: 'GET',
            data: {
              _token: token
            },
            ...
            if (result.value) {
              $.ajax({
                ...
                success: function (res) {
                  ...
                  toastr['warning']('👋 Advert Deleted Successfully', 'Deleted!', {
                    closeButton: true,
                    tapToDismiss: false,
                    rtl: isRtl
                  });
  ...
  if (newCategoryForm.length) {
    ...
    newCategoryForm.on('submit', function (e) {      
      ...
      if (isValid) {
        ...
        if(editForm){
          $.ajax({
            url: `${assetPath}api/v1/vert/${selectedRowDataId}?_method=PUT`,
            type: 'POST',
            ...
            success: function (res) {
              ...
              dtCategoryTableInit.ajax.reload();
              clearForm()
              toastr['info']('👋 Advert Updated Successfully.', 'Updated!', {
                closeButton: true,
                tapToDismiss: false,
                rtl: isRtl
              });
          ...
          $.ajax({
            ...
            success: function (res) {
              ...
              newCategorySidebar.modal('hide');
              toastr['success']('👋 Advert Created Successfully.', 'Success!', {
                closeButton: true,
                tapToDismiss: false,
                rtl: isRtl
              });
  ...
  $('[data-dismiss="modal"]').on('click', function(){      
    if(editForm){            
      $('#exampleModalLabel').text('New Advert');  
      editForm = false;      
      newCategoryForm.trigger('reset');
