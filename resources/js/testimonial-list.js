...
$(function () {
  ...
  if (dtCategoryTable.length) {
    var dtCategoryTableInit = dtCategoryTable.DataTable({
      ajax: {
        "url": `${assetPath}api/v1/testimonial`,
        "type": "GET",
        "error": function(err) {
          // console.log(err);
          Swal.fire({                            
      ...
      fnDrawCallback: function() {
        ...
        $('.formEdit').on('click', function(){ 
          ...
          editForm = true  
          if(editForm){            
            $('#exampleModalLabel').text('Edit Testimonial');  
          }        
          let id = $(this).attr('category_id');
          selectedRowDataId = id;
          $('#modals-slide-in').modal('show');
          $.ajax({
            url: `${assetPath}api/v1/testimonial/${id}`,
            type: 'GET',
            data: {
              _token: token
            },
            success: function (res) {
              ...
              $("input[name=user_designation]").val(res.data.user_designation);
            ...
            if (result.value) {
              $.ajax({
                ...
                success: function (res) {
                  ...
                  toastr['warning']('👋 Testimonial Deleted Successfully', 'Deleted!', {
                    closeButton: true,
                    tapToDismiss: false,
                    rtl: isRtl
                  });
  ...
  if (newCategoryForm.length) {
    ...
    newCategoryForm.on('submit', function (e) {
      ...
      if (isValid) {
        ...
        if(editForm){
          $.ajax({
            url: `${assetPath}api/v1/testimonial/${selectedRowDataId}?_method=PUT`,
            type: 'POST',
            ...
            success: function (res) {
              ...
              dtCategoryTableInit.ajax.reload();
              clearForm()
              toastr['info']('👋 Testimonial Updated Successfully.', 'Updated!', {
                closeButton: true,
                tapToDismiss: false,
                rtl: isRtl
              });
  ...
  $('[data-dismiss="modal"]').on('click', function(){      
    if(editForm){            
      $('#exampleModalLabel').text('New Testimonial');  
      editForm = false;      
      newCategoryForm.trigger('reset');
