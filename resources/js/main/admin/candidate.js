...
function enable(id, dom) {
    Swal.fire({
        title: 'Do you want Enable this candidate?',
        ...
        const url = "".concat(assetPath, "api/v1/admin/candidates/enable");
        Pace.track(()=> {
                $.ajax({
                    method: "POST",
                    url: url,
...
function continueED(data, id, dom, isEnable) {
    ...
    let d = '';
    if (isEnable == true) {
        d += '<buton title="Change Status" onclick="disable('+id+', this)" class="btn p-0 m-0"><i data-feather="toggle-right" class="text-success font-large-1"></i></buton>';
    } else {
...
function disable(id, dom) {
    Swal.fire({
        title: 'Do you want Block this candidate?',
        showCancelButton: true,
        confirmButtonText: `Yeah`,
    }).then((result) => {
        if (!result.isConfirmed) {
        ...
        const url = "".concat(assetPath, "api/v1/admin/candidates/disable");
        Pace.track(()=> {
                $.ajax({
                    method: "POST",
                    url: url,
...
function deleter(id, dom) {
    Swal.fire({
        title: 'Do you want remove this candidate?',
        showCancelButton: true,
        confirmButtonText: `Yeah`,
    }).then((result) => {
        if (!result.isConfirmed) {
        ...
        const url = "".concat(assetPath, "api/v1/admin/candidates/delete");
        Pace.track(()=>{
            $.ajax({
                method: "POST",
                url: url,
...
$(window).on('load', function () {
    ...
    table = $('#pageTable').DataTable({
        ...
        columnDefs: [
            {className: 'status-cell', targets: [3]},
            {className: 'action-cell', targets: [4]}
        ]
    });
});
